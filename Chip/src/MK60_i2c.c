...
//一次性读取MPU6050所有六轴的数据
void MPU6050_read_reg(I2Cn_e i2cn, uint8 SlaveID, uint8 reg,uint8 *buf)
{
    ...
    i = I2C_D_REG(I2CN[i2cn]);                     //虚假读取一次，启动接收数据
    i2c_Wait(i2cn);                                     //等待接收完成

    for(i=0;i<14;i++)
    {
      if(i==13) i2c_DisableAck(i2cn); 
      *(buf+i) = I2C_D_REG(I2CN[i2cn]);                     //读取数据
      i2c_Wait(i2cn);                                     //等待接收完成
    }
    i2c_Stop(i2cn);                                     //发送停止信号
    i2c_delay();                                        //必须延时一下，否则出错
...
//一次性读取HMC5883L所有的数据
void HMC5883L_read_reg(I2Cn_e i2cn, uint8 SlaveID, uint8 reg,uint8 *buf)
{
